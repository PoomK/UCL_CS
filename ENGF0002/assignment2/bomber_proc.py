...
def move_plane(pos):
    ...
    if pos[0] < -plane_width:
        pos[0] += CANVAS_WIDTH
        pos[1] = pos[1] + 40
        #we don't go off the bottom of the screen
...
def check_plane(canvas, plane_pos, building_width, building_heights, building_xpos):
    ...
    for building_num in range(0, 1200//SPACING):
        if (is_inside_building(building_num, plane_nose_pos, building_width,building_heights, building_xpos)
            or is_inside_building(building_num, plane_body_pos, building_width,building_heights, building_xpos)
            or is_inside_building(building_num, plane_wing_pos, building_width,building_heights, building_xpos)) :
            game_over(canvas)
    if plane_body_pos[1] >= CANVAS_HEIGHT and plane_body_pos[0] <= 1200:
        plane_landed(canvas)
...
def restart(canvas, plane_pos, building_heights, building_xpos, building_rects):
    global building_width, won, game_running, plane
    level = 1
    score = 0
    ...
    while prog_running:
        if seen_event:
            if ev.char == ' ':
                ...
                level = next_level(canvas, level, plane_pos, building_heights, building_xpos, building_rects)
            elif ev.char == 'r':
                (score, level) = restart(canvas, plane_pos, building_heights, building_xpos, building_rects)
            seen_event = False
